...
prepare_cache(){
  ...
  sed -i -e '/^  baremetal:/a \ \ \ \ clusterOSImage: ${CACHE_URL}/${MACHINE_OS_IMAGE_NAME}?sha256=${MACHINE_OS_IMAGE_SHA256}' ${INSTALLCONFIG}
  sed -i -e '/^  baremetal:/a \ \ \ \ bootstrapOSImage: ${CACHE_URL}/${MACHINE_OS_BOOTSTRAP_IMAGE_NAME}?sha256=${MACHINE_OS_BOOTSTRAP_IMAGE_UNCOMPRESSED_SHA256}' ${INSTALLCONFIG}
}
